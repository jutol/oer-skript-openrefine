...
Dort kann eingestellt werden, welche Spalten in die Prüfung auf Duplikate miteinbezogen werden. Zu Beginn werden alle Spalten einbezogen. 
...
Bei den Tabellenspalten **Lfd. Nr.** und **Fundstelle** könnte es sich um eindeutige Identifizierungsnummern handeln. Dafür wird eine Facettierung für jede der beiden Spalten durchgeführt und festgestellt, dass beide Spalten Werte aufweisen, die häufiger als einmal vorkommen. Beispiel: Die **Lfd. Nr.** `001` liegt dreifach vor, da sie in den Kampagnenjahren 2002, 2003 und 1987 vergeben wurde. Damit eignen sich beide Spalten nicht als eindeutige Identifizierungsnummern. Es muss also eine neue Spalte aus mehreren vorhandenen Spalten kreiert werden, um zu einer eindeutigen Identifizierungsnummer zu gelangen: Jeder Fund soll eine ID mit folgendem Schema erhalten: `Fundort (TCH) – Kampagnenjahr – Fundstelle – Lfd. Nr. – Einzelfundnummer bei Sammelfunden`
...
Auf die so entstandene Spalte kann wieder eine Facettierung angewendet werden. Nun wird auf einen Blick sichtbar, dass der Datensatz 10 unterschiedliche Kampagnenjahre umfasst. Duch klicken auf `Edit` neben den Werten `02` und `03` können alle Werte gleichzeitig auf das korrekte Kampagnenjahr `2002` bzw. `2003` geändert werden.
...
Nachdem nun eine Spalte mit den extrahierten Jahreszahlen vorliegt kann die ID aus mehreren Spalten konstruiert werden.
...
Der Spalte **Beschreibung** ist zu entnehmen, dass in manchen Fundstellen mehrere Funde geborgen wurden. Diese sogenannten Sammelfunde müssen für eine statistische Analyse in Einzelfunde aufgeteilt werden.

::: {.panel-tabset}

#### Arbeitsschritte

1.   Die Datensätze mithilfe der Filterfunktion `Text filter` heraus filtern, in denen die Beschreibung auf mehrere Funde in der selben Fundstelle hinweist (mithilfe eines regulären Ausdrucks wie `\d+`). TODO
...
Die ID aus `Fundort (TCH) – Kampagnenjahr – Fundstelle – Lfd. Nr.` sollte nun um die Zahl der einzelnen Funde aus einem Sammelfund erweitert werden.

Hierfür wird zunächst die Spalte **Zahl** facettiert und die Zahlen von 1–9 ausgewählt und durch `00` erweitert. Bei Zahlen größer als 10 wird nur eine führende `0` ergänzt.
...
#### Clustering der Fundbeschreibung
